        ...
        for (int i =0; i < products.length; i++) {
            if (products[i].equals(EMPTY)){
                ...
                for (int j =0; j <products.length; j ++) {
                    System.out.printf("> %s%n", products[j]);
                } return;
            }
...
//        int cnt = ;  // 수량 입력 받기
        ...
        for(char c: number.toCharArray()) {
            if(digit.indexOf(c) == -1) {
                ...
                counts[foundIdx] += Integer.parseInt(number);
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        for (int i =0; i < products.length ; i++) {
            if (input.equals(products[i])) {
                ...
                if(counts[i] >= numberInt) {
                    counts[i] -= numberInt;
                    System.out.println("[Clear] 출고가 완료되었습니다.");
                } else {
                    System.out.println("재고가 부족합니다. 현재 재고량 : " + counts[i]);
                }
                System.out.println("[System] 현재 등록된 물건 목록 및 수량 ▼");
                for (int j = 0; j < counts.length ; j++){
                    System.out.println(products[j] + ":" + counts[j]);
                }
                return;
            }
        }
